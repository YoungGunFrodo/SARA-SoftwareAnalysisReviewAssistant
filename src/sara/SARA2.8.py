...
class ChatGPTWorker(QThread):
    ...
    def run(self):
        ...
        # try up to 3 times with back-off
        for attempt in range(1, 4):
            try:
                self._log(f"Attempt {attempt}: sending request (timeout=60s)")
                resp = requests.post(
                    "https://api.openai.com/v1/chat/completions",
                    headers=headers,
                    json=payload,
                    timeout=60  # give it up to a full minute
                )
                ...
                if attempt < 3:
                    time.sleep(2 ** attempt)  # 2s, then 4s, then 8s
                    continue
                else:
                    # on last failure, capture full traceback
                    tb = traceback.format_exc()
                    response_text = f"ChatGPT error after {attempt} attempts: {e}\n\n{tb}"

        # finally emit something
        if response_text is None:
            response_text = "ChatGPT error: unknown failure"
        self.result.emit(response_text)

    def _log(self, msg: str):
        """Append msg to ~/.sara_logs/chatgpt_errors.txt with timestamp."""
        try:
            logdir = os.path.join(os.path.expanduser("~"), ".sara_logs")
            os.makedirs(logdir, exist_ok=True)
            path = os.path.join(logdir, "chatgpt_errors.txt")
...
- Any other concerns or insights about this software in a university setting 
        ...
        # store it on self so Python won't GC it while it's running
        self.llm_worker = LocalLLMWorker(prompt) if mode == "local" else ChatGPTWorker(prompt)
    ...
    def closeEvent(self, event):
        # Make sure any running threads finish before Qt tears them down
        for attr in ("llm_worker", "vt_thread", "abuse_thread"):
            thread = getattr(self, attr, None)
            if isinstance(thread, QThread):
                thread.quit()   # ask it to stop
                thread.wait()   # block until it’s fully done
